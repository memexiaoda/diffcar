...
class ControlCar():
    def __init__(self):
        ...
        self.angular_speed = rospy.get_param('Lasercontrol/ang_speed', 0.2)    # radians per second
        ...
        self.d_angular_speed = rospy.get_param('Lasercontrol/d_ang_speed', -0.2)    # radians per second
        ...
        self.MaxOutput = 0.1 #0.07           #PID最大误差值
    ...
    def Stop_Car(self,x):
        ...
        while move_cmd.linear.x > 0.0:
            #print "3333"
            #if self.min_speed < move_cmd.linear.x:
            #print "4444"
            move_cmd.linear.x = max(0.0,move_cmd.linear.x - 0.04)#0.02
            #print "-----"
    ...
    def d_keep_line(self,lastgoal,goal,work,stop,rfid,curfname):
        print "ControlThread::d_keep_line Lasermove start"
        self.ControlStatu = "start"
        ...
        path_rot = normalize_angle(path_rot - pi)
        #当前点距离目标的距离
        ...
        while not rospy.is_shutdown() and (goal_distance > 0):        
                    ...
                    if self.d_min_speed > move_cmd.linear.x:
                        ...
                        if distance_overplus > 0.018:
                            if distance > (goal_distance):
                                move_cmd.linear.x = 0.0
                                #print "ControlThread::d_keep_line d_kspeed 1-2-1 the linear.x = 0.0"
                            ...
                            move_cmd.linear.x = 0.0
                            #print "ControlThread::d_keep_line d_kspeed 1-2-2 the linear.x = 0.0"
                            #if abs(th_pid.Perror) < 0.01:
            ...
            if position == None or rotation == None:
                print "ControlThread::d_keepline postion rotation is None"
                self.Stop_Car(move_cmd.linear.x)
                self.Pubmsg()
            ...
            th_pid.Perror = normalize_angle(rotation + pi - path_rot)
#             if th_pid.Perror >= pi:
#                 th_pid.Perror -= pi*2
...
#                 th_pid.Perror = th_pid.Perror
        ...
        print "ControlThread::d_keep_line Lasermove done "
    ...
    def keep_line(self,lastgoal,goal,work,stop,rfid,curfname):
        print "ControlThread::keep_line Lasermove start"
        self.ControlStatu = "start"
        self.max_speed = rospy.get_param('LaserControl/max_speed', 0.8)                  #最大线速度
        self.min_speed = rospy.get_param('LaserControl/min_speed', 0.1)                  #最小线速度
        self.distance_slow =  rospy.get_param('LaserControl/distance_slow', 1.4)  
    ...
    def turn_angle(self,lastgoal,goal,curfname):
        print "ControlThread::turn_angle start" 
        self.ControlStatu = "start"
        move_cmd = Twist()
        position = Point()
        turn_angle = 0
...
#       self.sock.send_multipart([AllTopic[1], "\0", self.PublishMsg.SerializeToString()])
    ...
    def turn_angle_back(self,lastgoal,goal,curfname):
        print "ControlThread::turn_angle_back start" 
        self.ControlStatu = "start"
        move_cmd = Twist()
        position = Point()
        turn_angle = 0
        ...
        path_rot = self.cal_path_rotation(lastgoal.x,lastgoal.y,rotation,goal.x,goal.y)  
        path_rot = normalize_angle(path_rot - pi)
        goal_angle = normalize_angle(path_rot - rotation)                   
#         if goal_angle >= pi:
#             goal_angle -= (2*pi)
#             move_cmd.angular.z = -self.angular_speed
#         elif goal_angle < -pi:
...
#             move_cmd.angular.z = sign(goal_angle)*self.angular_speed
        ...
        self.Stop_Car(move_cmd.linear.x)
        self.ControlStatu = "end"
        print "ControlThread::turn_angle_back done" 
#       self.Pubmsg()
#       self.PublishMsg.To = RECVER[1]
#       self.PublishMsg.CallMessage.Function = ORDER[1]
#       self.PublishMsg.CallMessage.Parameters.append("laserdone")
